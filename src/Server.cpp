...
PageType Server::choosePageType(HttpMessage msg) {
    std::string decoded_path = html_decode(path + msg.address);
...
std::string Server::generateContent(HttpMessage msg) {
    ...
    auto decoded_path = html_decode(path+msg.address);
    if (pt == FileDataPage) {
        uintmax_t filesize = std::filesystem::file_size(decoded_path);
        auto [range_start, range_end] = msg.getRange(filesize)
                                        .value_or(std::make_pair(0, filesize));
        int status = range_end-range_start == filesize ? 200 : 206;
        std::string mimetype = get_mime_type(msg.address);
        std::string file_content = read_binary_to_string(decoded_path, range_start, range_end);
        ...
        std::string dir_page_contents = list_contents(html_decode(msg.address), decoded_path);
        std::string final_content = string_format(this->htmltemplate, decoded_path.c_str(), dir_page_contents.c_str());
        return HttpResponseBuilder()
            .Status(200)
