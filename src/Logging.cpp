...
Logger::Logger(std::string filename, bool wtf, bool wts, bool omit_metrics)
    : write_to_file(wtf), write_to_stdout(wts), no_metrics(omit_metrics) {
...
void Logger::exportMetrics() {
    ...
    while (getline(logs, line)) {
        if (line.ends_with("Starting Server")) {
            ...
            if (timestamp_del == std::string::npos) continue;
            auto time_str = line.substr(timestamp_del+2, metadata_end_del-timestamp_del-2);
            ...
            if (keyval_del == std::string::npos ||
                ...
                continue;
            auto timestamp = line.substr(timestamp_del+2, keyval_del-timestamp_del-2);
            auto keyval = line.substr(keyval_del+3);
            int time_int = 0;
            try {
                time_int = std::stoi(timestamp);
            } catch(std::invalid_argument& e) {
                std::cout << "Error: " << timestamp <<", "<< e.what() << std::endl;
                return;
            }
            auto key_del = keyval.find("=");
            if (key_del == std::string::npos) return;
            auto key = keyval.substr(0, key_del);
            auto val = keyval.substr(key_del+1);
            if (!datas.contains(key))
                datas[key] = std::to_string(time_int-start_timestamp) + " " + val + "\n";
            else
                datas[key] += std::to_string(time_int-start_timestamp) + " " + val + "\n";
        }
    }
...
void Logger::report(std::string type, std::string msg) {
    ...
    if (write_to_file)
        this->output_stream << "["<< type <<" @ " << time_str << "]: " << msg << std::endl;
    if (write_to_stdout)
